...
public class Flux {
    ...
    public static <T> Stream<T[]> fluxUplets(int taille, Supplier<T[]> depart, Function<T[], T[]> suivant) {
        AtomicReference<T[]> courant = new AtomicReference<>(depart.get());
        return Stream.concat(Stream.<T[]>of(courant.get()), IntStream.range(1, taille).mapToObj(index -> courant.updateAndGet(suivant::apply)));
    }
