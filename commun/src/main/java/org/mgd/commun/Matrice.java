...
public class Matrice<T> {
    ...
    public static Matrice<Float> projection60() {
        return Matrice.parValeurs(4,
                4,
                IConstantesMathematiques.RACINE_TROIS * 9f / 16f, 0f, 0f, 0f,
                0f, IConstantesMathematiques.RACINE_TROIS, 0f, 0f,
                0f, 0f, -1f, -2f,
                0f, 0f, -1f, 0f);
    }

    public static Matrice<Float> transformation(float[] translation, float[] agrandissement, float[] rotation) {
        float costhx = Double.valueOf(Math.cos(rotation[0])).floatValue();
        float costhy = Double.valueOf(Math.cos(rotation[1])).floatValue();
        float costhz = Double.valueOf(Math.cos(rotation[2])).floatValue();
        float sinthx = Double.valueOf(Math.sin(rotation[0])).floatValue();
        float sinthy = Double.valueOf(Math.sin(rotation[1])).floatValue();
        float sinthz = Double.valueOf(Math.sin(rotation[2])).floatValue();
        return Matrice.parValeurs(4,
                4,
